...
pending_outbound_router = APIRouter(prefix="/pending-outbounds", tags=["待出库单管理"])
...
async def execute_outbound(
    ...
    result = await pending_outbound_service.execute_outbound(pending_id, out_qty, operator)
    return result
...
async def batch_execute_outbound(
    items: List[Dict[str, Any]] = Body(..., description="出库明细列表 [{pending_id, out_qty}]"),
    current_user: dict = Depends(require_permission("warehouse.edit"))
):
    """批量执行出库操作"""
    operator = current_user.get("username", current_user.get("full_name", "system"))
